...
namespace Day3
{
    class Program
    {
        static void Main()
        {
            ...
            {
                char direction = wire1[i][0];
                int magnitude = Convert.ToInt32(wire1[i].Substring(1));
                totalMagnitude += magnitude;
                wire1Magnitude.Add(totalMagnitude + magnitude);

                switch (direction)
                {
                    case 'U':
                    up1.Add(up1[i] + magnitude);
                    right1.Add(right1[i]);
                        break;
            ...
            List<int> up2 = new List<int>() { 0 };
            List<int> right2 = new List<int>() { 0 };

            List<int> magnitudeArranged = new List<int>() { 0 };

            totalMagnitude = 0;

            foreach (string i in wire2)
            {
                char direction = i[0];
                int magnitude = Convert.ToInt32(i.Substring(1));

                int previousMagnitude = totalMagnitude;
                totalMagnitude += magnitude;
                ...
                up2.Add(currentUp);
                right2.Add(currentRight);

                int c = 0;
                if (oldUp == currentUp)
                {
                    int increase = (currentRight - oldRight) / Math.Abs(currentRight - oldRight);
                    if (increase == 1)
                    {
                        for (int x = oldRight; x <= currentRight; x++)
                        {
                            for (int y = 1; y < up1.Count; y++)
                            {
                                if ((right1[y] & right1[y - 1]) == x)
                                {
                                    if ((up1[y - 1] <= currentUp && up1[y] >= currentUp) || (up1[y - 1] >= currentUp && up1[y] <= currentUp))
                                    {
                                        int distance = Math.Abs(x) + Math.Abs(currentUp);
                                        Console.WriteLine(distance);
                                        intersections.Add(distance);

                                        int stepsWire1 = wire1Magnitude[y - 1] + Math.Abs(currentUp - up1[y - 1]);
                                        int stepsWire2 = previousMagnitude + c;
                                        int steps = stepsWire1 + stepsWire2;
                                        Console.WriteLine(steps);
                                        magnitudeArranged.Add(steps);
                                    }
                                }
                            }
                            c++;
                    ...
                    else if (increase == -1)
                    {
                        for (int x = oldRight; x >= currentRight; x--)
                        {
                            for (int y = 1; y < up1.Count; y++)
                            {
                                if ((right1[y] & right1[y - 1]) == x)
                                {
                                    if ((up1[y] <= currentUp && up1[y - 1] >= currentUp) || (up1[y] >= currentUp && up1[y - 1] <= currentUp))
                                    {
                                        int distance = Math.Abs(x) + Math.Abs(currentUp);
                                        Console.WriteLine(distance);
                                        intersections.Add(distance);
                ...
                {
                    int increase = (currentUp - oldUp) / Math.Abs(currentUp - oldUp);
                    if (increase == 1)
                    {
                        for (int x = oldUp; x <= currentUp; x++)
                        {
                            for (int y = 1; y < right1.Count; y++)
                            {
                                if ((up1[y] & up1[y - 1]) == x)
                                {
                                    if ((right1[y] <= currentRight && right1[y - 1] >= currentRight) || (right1[y] >= currentRight && right1[y - 1] <= currentRight))
                                    {
                                        int distance = Math.Abs(x) + Math.Abs(currentRight);
                                        Console.WriteLine(distance);
                                        intersections.Add(distance);

                                        int stepsWire1 = wire1Magnitude[y - 1] + Math.Abs(currentRight - right1[y - 1]);
                                        int stepsWire2 = previousMagnitude + c;
                                        int steps = stepsWire1 + stepsWire2;
                                        Console.WriteLine(steps);
                                        magnitudeArranged.Add(steps);
            ...
            Console.WriteLine("----------------------------------");
            foreach (int i in magnitudeArranged)
            {
                Console.WriteLine(i);
            }
            Console.WriteLine("----------------------------------");

            int lowest = intersections[1];
            int leastSteps = magnitudeArranged[2];
            for (int i = 2; i < intersections.Count; i++)
            {
                if (intersections[i] < lowest)
                {
                    lowest = intersections[i];
                }

                if (magnitudeArranged[i] < leastSteps)
                {
                    leastSteps = magnitudeArranged[i];
                }
            }
